...
if [ -f "$CLAUDE_CONFIG_FILE" ]; then
    ...
    config.mcpServers['coyote-user'] = {
        command: 'coyote-mcp-server'
    };
    
    fs.writeFileSync(path, JSON.stringify(config, null, 2));
    console.log('✅ Added coyote-user to existing Claude Desktop config');
    "
else
...
echo ""
echo "The coyote-user MCP server is now configured and ready to use!"
echo ""
